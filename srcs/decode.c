...
static int32_t		dis_sub_extract(size_t bytes, t_ocp *pc, int endian_mode)
{
	...
	if (bytes == 2)
	{
		if (vm_check_endianness(endian_mode))
			return (vm_endian_convert_16(*(int16_t *)value));
		return (*(int16_t *)value);
	}
	if (vm_check_endianness(endian_mode))
		return (vm_endian_convert_32(*(int32_t *)value));
...
static int		dis_extract_instr_data(t_single_instr *instr, t_din bcode, int index)
{
